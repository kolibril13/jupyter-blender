...
class Installer(Executor):
    ...
    def _target_removal_set(
        ...
        Install pulls in the whole dependency closure (tornado, pyzmq,
        jupyter_server, …), so a proper uninstall must remove that closure —
        not just the top-level dists, which would leave dozens of orphaned
        packages behind.
        """
        if not site_packages_path:
            return []
    ...
    def _unload_target_modules(
        ...
        already-imported ``jupyterlab`` stays importable until Blender
        restarts, and a later re-install would keep serving the stale
        in-memory modules instead of the fresh code. Must run *before* the
        uninstall subprocess, while each dist's file list / ``top_level.txt``
        is still on disk.
        """
